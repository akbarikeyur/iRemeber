...
class UploadImageVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, PECropViewControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let choosenImg : UIImage = info[UIImagePickerController.InfoKey.originalImage] as? UIImage
        {
            let controller : PECropViewController = PECropViewController()
            controller.delegate = self
            controller.image = choosenImg
...
//            UIApplication.shared.keyWindow?.topMostController()?.present(navigationController, animated: true, completion: {
//
//            })
